...
class AIAgent:
    ...
    def __init__(
        ...
        # 1) errors.log — WARNING+ only (compact error log)
        ...
        if self.verbose_logging:
            ...
            if self.quiet_mode:
                # Suppress console noise only — don't silence loggers at the
                # source or file handlers (hermes.log) stop receiving events.
                for handler in logging.getLogger().handlers:
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        logger.info("API_CALL model=%s mode=%s messages=%d tools=%d",
                     api_kwargs.get("model", self.model), self.api_mode,
                     len(api_kwargs.get("messages", [])), len(api_kwargs.get("tools", [])))
