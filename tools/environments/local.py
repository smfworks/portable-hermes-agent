...
def _get_embedded_python_path_prefix() -> str:
    ...
    Ensures `python` and `pip` resolve to python_embedded/ instead of
    system Python. Injected into the fenced command because Git Bash's
    login shell (-lic) rebuilds PATH from /etc/profile.
    """
